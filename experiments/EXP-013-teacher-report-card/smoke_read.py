#!/usr/bin/env python3
"""EXP-013 冒烟读数：从 5 计划×8 局 CSV 量单局耗时，外推全量预算并对照 job 超时。"""
...
def main() -> int:
    ...
    wall_h = per_cpu_h / 4  # runner 4 vCPU 并行
    per_shard_h = per_cpu_h / ci_jobs

    print(f"冒烟 n={n} 局（应为 40）  自选未达标={fails}")
    ...
    print(f"全量外推：单臂 {per_cpu_h:.1f} CPU·h；6 分片时每片 ≈ {per_shard_h*60:.0f} min 墙钟（4 vCPU 并行内已折算）")
    print(f"job 超时对照：每片 ≈ {per_shard_h*60:.0f} min vs bench job timeout 150 min "
          f"→ {'✅ 安全' if per_shard_h < 100 else '⚠️ 紧张，需加分片数'}")
    if fails > 0:
    ...
    Path("SMOKE.txt").write_text(
        ...
        f"full_single_arm_cpu_h={per_cpu_h:.2f} per_shard_min={per_shard_h*60:.0f}\n",
        encoding="utf-8",
    )
    return 0
